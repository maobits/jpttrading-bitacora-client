...
export async function fetchPortfolioProfitability(apiResponse: ApiResponse) {
  try {
    ...
    console.log(
      "URL para conectar con el servidor de calculo aritmetico y lógico:",
      baseURL
    );
    ...
    const requestData = apiResponse.results
      .map((position) => {
        ...
        try {
          ...
          console.error(
            `❌ Error al parsear JSON en posición ID ${position.id}:`,
            error
          );
          return null;
        }
        ...
        const tipoPosicion =
          position.TradeDirection === "Buy" ? "largo" : "corto";
        ...
        const transacciones = priceEntries
          ...
          .map((entry: any) => {
            ...
            const porcentaje = allocation.activeAllocation
              ? parseFloat(allocation.activeAllocation) / 100
              : 0;
            const tipo =
              entry.type === "add"
                ? "adicion"
    ...
    console.log(
      "📤 Objeto generado antes de enviar par el calculo del portafolio:",
      JSON.stringify(requestData, null, 2)
    );

    if (requestData.length === 0) {
      console.error(
        "❌ No hay datos válidos para enviar al servidor en el calculo del portafolio."
      );
      return { error: "No hay datos válidos para calcular el portafolio." };
    }

    console.log(
      "🚀 Enviando solicitud al servidor para el calculo del portafolio:",
      JSON.stringify(requestData, null, 2)
    );
    ...
    try {
      ...
      console.log("✅ Respuesta recibida para el calculo de portafolio:", data);
      return data;
    } catch (error) {
      console.error(
